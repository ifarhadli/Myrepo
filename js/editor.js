...
(function(){
  ...
  function renderMediaDetail(root,item){
    ...
    var isLogo=state.mediaContext&&state.mediaContext.kind==='slot'&&state.mediaContext.key==='shared.logo';detail.classList.toggle('omni-logo-detail',!!isLogo);
    if(isLogo){$('[for=omniMediaAlt]',detail).textContent='Logo name';$('#omniMediaAltHelp',detail).textContent='The business name announced by screen readers. It starts with your website name.';$('[data-media-alt]',detail).value=item.alt||state.draft.settings.siteName||'OmniMark';var note=document.createElement('p');note.className='omni-panel__hint';note.textContent='Your full logo keeps its proportions. It is never cropped.';$('.omni-media-detail-preview',detail).after(note);var sizeField=document.createElement('div');note.after(sizeField);logoSizeField(sizeField);}
  ...
  function openMedia(context){finishEdit(true);state.mediaContext=context||null;if(context)state.mediaSearch='';state.mediaSelected=currentMediaValue()||null;state.mediaStep=context&&state.mediaSelected?'edit':'choose';var panel=openPanel(context?(context.key==='shared.logo'?'Change logo':'Change image'):'Image library','left',renderMediaPanel),root=$('.omni-panel__body',panel);panel.setAttribute('data-media-panel','');loadMedia().then(function(){if($('.omni-panel')===panel)renderMediaPanel(root);}).catch(function(error){root.innerHTML='<div class="omni-panel-state">Could not load the media library.<br><button type="button">Try again</button></div>';$('button',root).addEventListener('click',function(){openMedia(context);});toast(error.message||'Could not load the media library.','error');});}
  ...
  function applyDraftSettings(){
    ...
    if(window.OmniSite)window.OmniSite.applyBrand(state.draft);
    $$('.lang-switch').forEach(function(control){control.hidden=state.draft.features.langSwitch===false||state.draft.features.englishVersion===false;});
    var english=$('[data-owner-english]');if(english&&english.checked!==(state.draft.features.englishVersion!==false)&&english.closest('.omni-owner-languages').refresh)english.closest('.omni-owner-languages').refresh();
    var construction=$('[data-owner-construction]');if(construction)construction.checked=!!state.draft.features.underConstruction;
    $$('[data-owner-logo-size]').forEach(function(input){input.value=settings.logoSize||'standard';});
  ...
  function logoSizeField(root){if(!state.permissions.manageSettings)return;var input=settingField(root,{path:'settings.logoSize',label:'Logo size',type:'select',options:[['standard','Standard'],['large','Large'],['extra-large','Extra large']]});input.value=state.draft.settings.logoSize||'standard';input.setAttribute('data-owner-logo-size','');}
  function openWebsiteControl(selector){
    finishEdit(true);clearSelection();if(!state.permissions.manageSettings){toast('Website language settings require the Admin role.','error');return;}
    var drawer=$('#mobileDrawer.open');if(drawer)$('#drawerClose').click();
    var panel=openPanel('Website details','right',renderWebsiteDetails),input=$(selector,panel);if(input){input.scrollIntoView({block:'center'});input.focus({preventScroll:true});}
  }
  function renderWebsiteDetails(root){
    ...
    settingField(form,{path:'settings.siteName',label:'Website name'});
    var languages=document.createElement('div');languages.className='omni-owner-languages';form.appendChild(languages);
    function renderLanguages(){
      languages.textContent='';var english=state.draft.features.englishVersion!==false;
      var toggle=document.createElement('label');toggle.className='omni-switch';toggle.innerHTML='<input type="checkbox" data-owner-english data-field-explained aria-describedby="omniEnglishHelp"><span><strong>Offer English version</strong><small id="omniEnglishHelp" class="omni-field-help">Turn off to show the public website only in Azerbaijani. English content is kept for later. Changes take effect after publishing.</small></span>';
      var input=$('input',toggle);input.checked=english;input.addEventListener('change',function(){commit(function(d){d.features.englishVersion=input.checked;if(!input.checked)d.settings.defaultLang='az';},'Change English availability');renderLanguages();$('[data-owner-english]',languages).focus();});languages.appendChild(toggle);
      if(english)settingField(languages,{path:'settings.defaultLang',label:'Website language',type:'select',options:[['en','English'],['az','Azərbaycanca']]});
      else{var hint=document.createElement('p');hint.className='omni-panel__hint';hint.textContent='Public language: Azərbaycanca. You can still edit the saved English content in Edit mode.';languages.appendChild(hint);}
      if(window.OmniFieldHelp)window.OmniFieldHelp.enhance(languages);
    }
    languages.refresh=renderLanguages;renderLanguages();
    var logo=document.createElement('div');logo.className='omni-owner-logo';logo.innerHTML='<h3>Website logo</h3><p class="omni-panel__hint">Used in the header, mobile menu, footer and construction screen. Upload a transparent PNG for best results. The whole logo is shown without cropping.</p><button type="button" class="omni-panel-action" data-owner-logo>Choose or replace logo</button>';form.insertBefore(logo,languages);logoSizeField(logo);$('[data-owner-logo]',logo).addEventListener('click',function(){openMedia({kind:'slot',key:'shared.logo'});});
    var construction=document.createElement('label');construction.className='omni-switch';construction.innerHTML='<input type="checkbox" data-owner-construction data-field-explained aria-describedby="omniConstructionHelp"><span><strong>Under construction</strong><small id="omniConstructionHelp" class="omni-field-help">After publishing, visitors see a temporary Azerbaijani page with your logo and contact email. You can still sign in, edit and use private previews. Turn off and publish to reopen the website.</small></span>';
    var availability=$('input',construction);availability.checked=!!state.draft.features.underConstruction;availability.addEventListener('change',function(){commit(function(d){d.features.underConstruction=availability.checked;},availability.checked?'Show construction page':'Reopen website');});form.appendChild(construction);
    [['settings.email','Contact email','email'],['settings.phone','Phone','tel'],['settings.addressLine1','Street address'],['settings.addressLine2','City and country'],['settings.linkedin','LinkedIn page','url']].forEach(function(f){settingField(form,{path:f[0],label:f[1],type:f[2]});});
  ...
  function showElementTools(info){
    var old=$('.omni-element-tools');if(old)old.remove();if(!info||state.activeEdit||info.target.closest('.lang-switch'))return;
    ...
    var isBrand=!!info.target.closest('.mark');
    if(!isBrand)tools.appendChild(removalButton(info));
    if(isBrand&&state.permissions.manageSettings){var logo=document.createElement('button');logo.type='button';logo.textContent='Change logo';logo.setAttribute('data-edit-logo','');logo.addEventListener('click',function(){openMedia({kind:'slot',key:'shared.logo'});});tools.appendChild(logo);}
  ...
  function bindEvents(){
    ...
    document.addEventListener('click',function(e){if(e.target.closest('.lang-switch')){e.preventDefault();e.stopImmediatePropagation();openWebsiteControl('[data-owner-english]');}},true);
