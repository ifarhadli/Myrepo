...
(function(){
  ...
  var state = { live:null, draft:null, savedAt:null, lang:'en', undo:[], redo:[], activeEdit:null, saveTimer:null, saveSeq:0, saving:false, defaults:null, draggedSection:null, draggedItem:null, panelReturnFocus:null, allowNavigate:false, submissions:null, pagesInfo:null, media:null, mediaContext:null, mediaSearch:'', mediaSelected:null, uploads:[] };
  var COLOR_TOKENS = [
  ...
  function applyDraft(){
    if(window.OmniSite){window.OmniSite.applyDesign(state.draft);window.OmniSite.applyLayout(state.draft);window.OmniSite.applyImages(state.draft);}
    mergeDraftDict();if(window.OmniI18n)window.OmniI18n.applyI18n();renderCatalogueLists();renderIndustryStrip();applyDraftSettings();decorateSections();decorateItems();decorateImageSlots();syncDesignPanel();updateBar();
  }
  ...
  function editableTarget(target){
    if(target.closest&&target.closest('.omni-bar,.omni-panel,.omni-dialog,.omni-section-tools,.omni-item-tools,.omni-catalogue-add,.omni-image-action'))return null;
  ...
  function mediaContextLabel(){var context=state.mediaContext;if(!context)return'Library';if(context.kind==='slot')return context.key;if(context.kind==='page-social')return PAGE_LABELS[context.key]+' social image';return'Site-wide social image';}
  function currentMediaValue(){var context=state.mediaContext;if(!context)return'';if(context.kind==='slot')return((state.draft.images||{})[context.key]||{}).id||'';if(context.kind==='page-social')return getPath(state.draft,'pages.'+context.key+'.ogImage')||'';return getPath(state.draft,'settings.ogImage')||'';}
  ...
  function assignMedia(item){
    var context=state.mediaContext;if(!context||!item)return;commit(function(draft){if(context.kind==='slot'){draft.images=draft.images||{};draft.images[context.key]={id:item.id,alt:item.alt||'',focal:clone(item.focal||{x:.5,y:.5})};}else if(context.kind==='page-social'){draft.pages=draft.pages||{};draft.pages[context.key]=draft.pages[context.key]||{};draft.pages[context.key].ogImage=item.id;}else{draft.settings=draft.settings||{};draft.settings.ogImage=item.id;}},'Use image for '+mediaContextLabel());toast('Image assigned to '+mediaContextLabel()+'.');
  }
  function removeMediaAssignment(){var context=state.mediaContext;if(!context)return;commit(function(draft){if(context.kind==='slot'&&draft.images)delete draft.images[context.key];else if(context.kind==='page-social'&&draft.pages&&draft.pages[context.key])delete draft.pages[context.key].ogImage;else if(context.kind==='site-social'&&draft.settings)delete draft.settings.ogImage;},'Remove image from '+mediaContextLabel());refreshMediaPanel('[data-media-use]');}
  function mediaUses(item){var refs=(item.usedBy||[]).filter(function(ref){return ref.indexOf('draft:')!==0;}),id=item.id,draft=state.draft||{};Object.keys(draft.images||{}).forEach(function(key){if(draft.images[key]&&draft.images[key].id===id)refs.push('draft:'+key);});if(draft.settings&&draft.settings.ogImage===id)refs.push('draft:settings.ogImage');Object.keys(draft.pages||{}).forEach(function(key){if(draft.pages[key]&&draft.pages[key].ogImage===id)refs.push('draft:pages.'+key+'.ogImage');});return refs.filter(function(ref,index){return refs.indexOf(ref)===index;});}
  function previewFocal(item,x,y,root){var value=x+'% '+y+'%';$$('[data-media-preview]',root).forEach(function(img){img.style.objectPosition=value;});if(state.mediaContext&&state.mediaContext.kind==='slot'){var slot=$('[data-image="'+state.mediaContext.key+'"]');if(slot&&currentMediaValue()===item.id)slot.style.objectPosition=value;}$('[data-focal-x]',root).value=x;$('[data-focal-y]',root).value=y;var cross=$('.omni-focal__cross',root);if(cross){cross.style.left=x+'%';cross.style.top=y+'%';}}
  function saveMediaDetails(item,root,after){
    ...
    api('PATCH','/api/media/'+item.id,{name:name,alt:alt,focal:{x:x/100,y:y/100}}).then(function(result){replaceMedia(result.item);if(state.mediaContext&&state.mediaContext.kind==='slot'&&currentMediaValue()===item.id){commit(function(draft){draft.images[state.mediaContext.key].alt=result.item.alt;draft.images[state.mediaContext.key].focal=clone(result.item.focal);},'Update image details');}toast('Image details saved.');if(after)after(result.item);else refreshMediaPanel('[data-media-save]');}).catch(function(error){button.disabled=false;button.textContent='Save details';toast(error.message||'Could not save image details.','error');});
  }
  ...
  function renderMediaDetail(root,item){
    ...
    $('[data-media-save]',detail).addEventListener('click',function(){saveMediaDetails(item,detail);});var use=$('[data-media-use]',detail),remove=$('[data-media-remove]',detail),assigned=currentMediaValue()===item.id;use.hidden=!state.mediaContext;remove.hidden=!assigned;use.textContent=assigned?'Use selected details':'Use here';use.addEventListener('click',function(){var alt=$('[data-media-alt]',detail).value.trim();if(!alt&&state.mediaContext.kind==='slot'){$('[data-media-alt]',detail).setAttribute('aria-invalid','true');toast('Add alt text before using this image.','error');$('[data-media-alt]',detail).focus();return;}saveMediaDetails(item,detail,function(saved){assignMedia(saved);refreshMediaPanel('[data-media-use]');});});remove.addEventListener('click',removeMediaAssignment);var del=$('[data-media-delete]',detail);del.disabled=used.length>0;del.title=used.length?'In use by '+used.join(', '):'';del.addEventListener('click',function(){deleteMedia(item);});
  }
  ...
  function openThisPage(){finishEdit(true);openPanel('This page','right',renderPagePanel);}

  function closePanel(){var panel=$('.omni-panel');if(!panel)return;panel.remove();if(window.OmniSite&&state.draft)window.OmniSite.applyImages(state.draft);if(state.panelReturnFocus&&state.panelReturnFocus.isConnected)state.panelReturnFocus.focus();state.panelReturnFocus=null;}
  ...
  function missingAltSlots(){return Object.keys((state.draft&&state.draft.images)||{}).filter(function(key){var value=state.draft.images[key];return value&&value.id&&!String(value.alt||'').trim();});}
  function publish(){
    finishEdit(true);if(!totalChanges())return;var missing=missingAltSlots();if(missing.length){var list='<p>Every meaningful image needs alt text before it can go live. Add it for:</p><ul>'+missing.map(function(key){return'<li>'+escapeHtml(key)+'</li>';}).join('')+'</ul>';return openDialog({title:'Add missing alt text',html:list,confirm:'Open first image',action:function(button,dialog){dialog.close('media');setTimeout(function(){openMedia({kind:'slot',key:missing[0]});},0);}});}var counts=summaryCounts(),labels={texts:'Texts',sections:'Sections',items:'Items',catalogue:'Catalogue',images:'Images',design:'Design',settings:'Settings'};
  ...
  function buildBar(){
    ...
    $('[data-editor-page]').addEventListener('click',openThisPage);$('[data-editor-media]').addEventListener('click',function(){openMedia(null);});$('[data-editor-design]').addEventListener('click',function(){openPanel('Design','left',renderDesignPanel);});$('[data-editor-inbox]').addEventListener('click',openInbox);$('[data-editor-settings]').addEventListener('click',openSettings);$('[data-editor-history]').addEventListener('click',openHistory);$('[data-editor-phone]').addEventListener('click',togglePhone);
  }
