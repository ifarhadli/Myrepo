...
(function(){
  ...
  var state = { live:null, draft:null, savedAt:null, lang:'en', undo:[], redo:[], activeEdit:null, saveTimer:null, saveSeq:0, saving:false, defaults:null, draggedSection:null, draggedItem:null, panelReturnFocus:null, allowNavigate:false, submissions:null, pagesInfo:null };
  var COLOR_TOKENS = [
  ...
  function renderPagePanel(root){
    ...
    function preview(){
      ...
      var src=image.value.trim()||((state.draft.settings&&state.draft.settings.ogImage)||'');if(!src){previewImage.hidden=true;shareEmpty.hidden=false;shareEmpty.textContent='No social image';}
    ...
    [title,description,image].forEach(function(input){input.addEventListener('input',preview);});title.addEventListener('change',function(){write('title',title.value.trim(),'Update page title');});description.addEventListener('change',function(){write('description',description.value.trim(),'Update page description');});image.addEventListener('change',function(){var value=image.value.trim();formMessage(root,'');if(value&&!/^https?:\/\//i.test(value)){image.setAttribute('aria-invalid','true');formMessage(root,'Use a complete http:// or https:// image URL.','error');image.focus();return;}image.removeAttribute('aria-invalid');write('ogImage',value,'Update page social image');});noindex.addEventListener('change',function(){write('noindex',noindex.checked,'Change search visibility');preview();});
  ...
  function buildBar(){
    ...
    $('[data-editor-page]').addEventListener('click',openThisPage);$('[data-editor-design]').addEventListener('click',function(){openPanel('Design','left',renderDesignPanel);});$('[data-editor-inbox]').addEventListener('click',openInbox);$('[data-editor-settings]').addEventListener('click',openSettings);$('[data-editor-history]').addEventListener('click',openHistory);$('[data-editor-phone]').addEventListener('click',togglePhone);
  }
