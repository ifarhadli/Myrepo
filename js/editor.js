...
(function(){
  ...
  function renderMobileMenu(root){
    ...
    root.innerHTML='<div class="omni-mobile-menu"><div class="omni-field"><label for="omniMobilePageSelect">Page</label><select id="omniMobilePageSelect">'+options+'</select></div><div class="omni-mobile-menu__language"><span>Language</span><div class="omni-segments" role="group" aria-label="Editing language"><button type="button" data-mobile-lang="en">EN</button><button type="button" data-mobile-lang="az">AZ</button></div></div><div class="omni-mobile-menu__actions"><button type="button" data-mobile-open="design">Design</button><button type="button" data-mobile-open="page">This page</button><button type="button" data-mobile-open="media">Media</button><button type="button" data-mobile-open="history">History</button><button type="button" data-mobile-open="inbox">Inbox <span data-unread></span></button><button type="button" data-mobile-open="settings">Settings</button><button type="button" data-mobile-open="discard" data-danger>Discard draft</button></div><p class="omni-mobile-menu__status" data-editor-status aria-live="polite">'+escapeHtml(state.savedAt?'Draft restored':'Draft ready')+'</p></div>';
    ...
    root.addEventListener('click',function(event){var button=event.target.closest('[data-mobile-open]');if(!button)return;var action=button.getAttribute('data-mobile-open');if(action==='design')openPanel('Design','left',renderDesignPanel);if(action==='page')openThisPage();if(action==='media')openMedia(null);if(action==='history')openHistory();if(action==='inbox')openInbox();if(action==='settings')openSettings();if(action==='discard'){closePanel();discard();}});
  }
