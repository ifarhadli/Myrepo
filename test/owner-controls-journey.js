...
module.exports=async function({evaluate:ev,go,viewport,check,screenshot,pause,send},width){
  ...
  await click('.omni-panel__close');await settings();await screenshot('owner-controls-'+width);
  ...
  await ev(`new Promise(resolve=>{const c=document.createElement('canvas');c.width=800;c.height=200;const x=c.getContext('2d');x.fillStyle='#4634f0';x.fillRect(0,0,800,200);x.fillStyle='#fff';x.font='bold 100px sans-serif';x.fillText('TEST LOGO',40,135);c.toBlob(blob=>{const input=document.querySelector('#omniMediaFiles'),dt=new DataTransfer();dt.items.add(new File([blob],'owner-logo.png',{type:'image/png'}));input.files=dt.files;input.dispatchEvent(new Event('change',{bubbles:true}));resolve();},'image/png');})`);
