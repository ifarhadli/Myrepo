...
async function main(){
  ...
  check('Services is a direct link without a dropdown', await evaluate(`!document.querySelector('.nav-item.has-mega')&&document.querySelector('.primary-nav a[href="services.html"]').getAttribute('aria-haspopup')===null`));
  await evaluate(`document.querySelector('.primary-nav a[href="services.html"]').click()`);
  await waitFor(()=>evaluate(`location.pathname==='/services.html'`),5000);
  check('desktop Services click opens the services page',await evaluate(`location.pathname==='/services.html'`));
  ...
  if(process.env.OWNER_CONTROLS_ONLY==='1'){
    for(const width of [1366,390])await require('./owner-controls-journey')({evaluate,go,viewport,check,screenshot,pause,send},width);
    ws.close();console.log(pass+' browser checks passed, '+fail+' failed');return;
  }
  await require('./add-element-journey')({evaluate,go,viewport,check,screenshot,pause},1366);
  await require('./move-element-journey')({evaluate,go,viewport,check,screenshot,pause,send},1366);
  ...
  await evaluate(`document.querySelector('[data-i18n="home.hero.micro"]').click()`);
  check('one click selects a paragraph without entering text edit',await evaluate(`!!document.querySelector('.omni-element-tools')&&!document.querySelector('[data-i18n="home.hero.micro"]').isContentEditable`));
  await evaluate(`document.querySelector('.omni-element-tools [data-element-remove]').click()`);
  check('Remove fades the selected element and explains restoration',await evaluate(`(()=>{const el=document.querySelector('[data-i18n="home.hero.micro"]');return el.getAttribute('data-omni-hidden-element')==='index:home.hero.micro'&&getComputedStyle(el).opacity==='0.4'&&getComputedStyle(el,'::after').content.includes('restore');})()`));
  await evaluate(`document.querySelector('[data-i18n="home.hero.micro"]').scrollIntoView({block:'center',behavior:'instant'})`);await screenshot('element-removed-1366');
  await evaluate(`document.querySelector('[data-editor-lang="az"]').click()`);
  check('removed elements stay removed when switching language',await evaluate(`document.querySelector('[data-i18n="home.hero.micro"]').hasAttribute('data-omni-hidden-element')`));
  await evaluate(`document.querySelector('[data-editor-lang="en"]').click()`);
  await evaluate(`document.querySelector('[data-editor-undo]').click()`);
  check('Undo restores an element without losing its text',await evaluate(`!document.querySelector('[data-i18n="home.hero.micro"]').hasAttribute('data-omni-hidden-element')&&document.querySelector('[data-i18n="home.hero.micro"]').textContent.includes('No deck')`));
  await evaluate(`document.body.dispatchEvent(new KeyboardEvent('keydown',{key:'Delete',bubbles:true}))`);
  check('Delete removes a selected element outside text edit',await evaluate(`window.OmniEditor.getState().draft.hiddenElements.includes('index:home.hero.micro')`));
  await evaluate(`document.querySelector('.omni-element-tools [data-element-remove]').click();document.body.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true}));`);
  check('Restore and Escape bring the element back and clear selection',await evaluate(`!document.querySelector('[data-omni-hidden-element]')&&!document.querySelector('.omni-element-tools')&&!document.querySelector('.omni-selected')`));
  await evaluate(`document.querySelector('[data-i18n="home.hero.h1"]').click()`);
  check('page headline removal is disabled with its reason',await evaluate(`document.querySelector('[data-element-remove]').disabled&&document.querySelector('[data-element-remove]').title.includes('Every page needs a headline')`));
  ...
  await evaluate(`(() => { const el=document.querySelector('[data-i18n="home.hero.h1"]'); el.click(); document.querySelector('[data-element-edit]')?.click(); el.textContent='A sharper draft headline.'; el.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true})); })()`);
  ...
  await evaluate(`(() => { const el=document.querySelector('[data-i18n="home.hero.eyebrow"]'); const before=el.textContent; el.click(); document.querySelector('[data-element-edit]')?.click(); el.textContent='Temporary eyebrow'; el.dispatchEvent(new KeyboardEvent('keydown',{key:'Escape',bubbles:true})); return {before,after:el.textContent,editing:el.hasAttribute('contenteditable')}; })()`);
  ...
  await evaluate(`document.querySelector('.eng-row[data-n="1"] .eng-cols li .omni-item-remove').click()`);
  state = await evaluate(`({ count:document.querySelectorAll('.eng-row[data-n="1"] .eng-cols li').length, hidden:document.querySelector('.eng-row[data-n="1"] .eng-cols li').hasAttribute('data-omni-hidden-element') })`);
  check('catalogue removal preserves its content for restoration', state.count===catalogueBefore&&state.hidden, JSON.stringify(state));
  ...
  state = await evaluate(`document.querySelector('.eng-row[data-n="1"] .eng-cols li').offsetParent===null`);
  check('published catalogue removal hides the service bullet for visitors',state);
  ...
  check('switching slots during Use here preserves the original destination', await evaluate(`!window.OmniEditor.getState().draft.images?.['index.logos.l1']?.id&&window.OmniEditor.getState().mediaContext.key==='index.logos.l1'`));
  ...
  check('Apply requires an image description and leaves the draft unchanged on error',await evaluate(`document.querySelector('[data-media-alt]').getAttribute('aria-invalid')==='true'&&!window.OmniEditor.getState().draft.images?.['index.logos.l1']?.id`));
  ...
  await waitFor(() => evaluate(`window.OmniEditor.getState().draft.images?.['index.logos.l1']?.alt==='Coral case artwork with a dark circle'`), 8000);
  ...
  await viewport(390,844);await go('/index.html');
  state = await evaluate(`(() => {const first=document.querySelector('[data-image="index.hero"]'),second=document.querySelector('[data-image="index.logos.l1"]'),empty=document.querySelector('[data-image="index.logos.l2"]');return{overflow:document.documentElement.scrollWidth-innerWidth,first:{hidden:first.hidden,srcset:first.srcset,position:first.style.objectPosition,alt:first.alt},second:{hidden:second.hidden,srcset:second.srcset,position:second.style.objectPosition,alt:second.alt,saved:window.OMNI_SITE.images['index.logos.l1']},empty:{hidden:empty.hidden,fallback:!empty.parentElement.classList.contains('has-slot-image')}};})()`);
  check('published hero preserves crop while unapproved proof images stay hidden', state.overflow <= 0 && !state.first.hidden && /-1600\.webp 1600w/.test(state.first.srcset) && state.first.position === '25% 68%' && state.first.alt === 'Purple and lime case-study artwork' && state.second.hidden && state.second.saved.focal.x === .72 && state.second.saved.focal.y === .29, JSON.stringify(state));
  ...
  check('in-use media cannot be deleted and explains where it is used', !!deleteReady && state.deleteDisabled && /index\.hero|index\.logos\.l1/.test(state.used), JSON.stringify(state));
  await evaluate(`document.querySelector('.omni-panel__close').click()`);
  ...
  await evaluate(`(() => {const title=document.querySelector('[data-field="title"]');title.click();document.querySelector('[data-element-edit]')?.click();title.textContent='Browser Growth Case';title.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));document.querySelector('[data-editor-lang="az"]').click();})()`);await pause(100);
  await evaluate(`(() => {const title=document.querySelector('[data-field="title"]');title.click();document.querySelector('[data-element-edit]')?.click();title.textContent='Browser AZ Case';title.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));document.querySelector('[data-editor-lang="en"]').click();})()`);await pause(100);
  ...
  await evaluate(`document.querySelector('[data-field="body"]').click();document.querySelector('[data-element-edit]')?.click()`);await pause(80);state=await evaluate(`({ formats:[...document.querySelectorAll('.omni-mini-tools [data-format]')].map(x=>x.dataset.format), editable:document.querySelector('[data-field="body"]').isContentEditable })`);
  ...
  await evaluate(`(() => {const el=document.querySelector('[data-i18n="home.hero.h1"]');el.click(); document.querySelector('[data-element-edit]')?.click();el.textContent='Mobile editor headline.';})()`);await pause(100);
  ...
  await require('./owner-controls-journey')({evaluate,go,viewport,check,screenshot,pause,send},1366);
  await require('./owner-controls-journey')({evaluate,go,viewport,check,screenshot,pause,send},390);
